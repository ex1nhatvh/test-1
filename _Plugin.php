...
class Plugin extends PluginPageBase{
	...
	public function requestSave($furniture_type,$request_type){
		...
		$request=request()->all();
		FMSRequest::save($furniture_type,$request_type,$request);
		admin_toastr('申請しました');
		
		return redirect(admin_url($this->getRouteUri('request'))."?f=".$furniture_type."&r=".$request_type);
